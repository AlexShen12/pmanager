...
def update_cred(cred_u: schemas.CredentialUpdate, db: Session = Depends(get_db)):
    ...
    update_user = crud.update_cred(db, cred_u = cred_u.subusername, cred_update = cred_u) # need to change request formatting.
    return update_user

@cred_router.delete("/delete/", response_model = schemas.DelResponse) 
# how do websites get this on the same page as update user 
def delete_cred(cred_u: schemas.CredentialResponse, db: Session = Depends(get_db)):
    if not cred_u:
        raise HTTPException(status_code = 400, detail = "Unable to delete user.")
    del_user = crud.delete_user(db, cred_u.id)
    return del_user
